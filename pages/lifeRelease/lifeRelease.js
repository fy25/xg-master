...
Page({
  ...
  getSalaryList() {
    let params = {
      type: 'POST',
      url: 'LifeServicesItemList.aspx',
      data: {},
      sCallback: (res) => {
        console.log(res)
        let lefeText = [];
        res.data.forEach(item => {
          lefeText.push(item.name)
        });
        this.setData({
          lifeList: res.data,
          lefeText: lefeText,
          lifeServicesItemId: res.data[0].id
        })
      }
    }
    app.request(params);
  ...
  changeSalary(e) {
    this.setData({
      ...
      lifeServicesItemId: this.data.lifeList[e.detail.value].id
    })
  },
  ...
  submitTap() {
    ...
    let lifeServicesItemId = this.data.lifeServicesItemId;
    if (describe == null || name == null || mobile == null) {
      ...
      setTimeout(() => {
        ...
        if (stringImageList == null) {
          ...
          let params = {
            type: 'POST',
            url: 'ReleaseLifeServices.aspx',
            data: {
              'uid': uid,
              'lifeServicesItemId': lifeServicesItemId,
              'describe': describe,
